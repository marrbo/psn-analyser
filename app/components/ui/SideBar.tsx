...
import { FaClock, FaGamepad, FaPlaystation } from "react-icons/fa";
...
export default function SideBar({ show }: { show?: boolean }) {
  ...
  const totalTrophies =
    (focusGame?.definedTrophies.bronze || 0) +
    (focusGame?.definedTrophies.silver || 0) +
    (focusGame?.definedTrophies.gold || 0) +
    (focusGame?.definedTrophies.platinum || 0);

  const platinum = focusGame?.trophyGroups[0]?.trophies.find(
  ...
  const playedTime = focusGame?.gameTitle?.playDuration || "0";
  const showGameInfo = globalThis?.location?.pathname?.includes("/dashboard/") || false;

  let lastPlayed = focusGame?.gameTitle?.lastPlayedDateTime ? 
    formatDistanceToNow(new Date(focusGame?.gameTitle?.lastPlayedDateTime || '1970-01-01T00:00:00.000Z'), {
      locale: ptBR,
      ...
      >
        ...
        <div className="bg-radial from-transparent via-65% to-black/70 p-4 w-100 h-100 border border-white/20 
          ...
          <div className="flex items-center justify-between mb-13">
            <span className="flex items-center gap-1 font-bold"><FaGamepad className="w-4 h-4 text-white"/> Jogado recentemente</span>
          ...
          <AutoTrimImage
              ...
              maskImage={false}
              position="center"
              className="w-full scale-95 h-25 mb-12 px-3 object-cover-custom"
            />
          
          <span className="font-bold text-lg">{focusGame?.trophyTitleName}</span>

          <div className="flex flex-col mt-2 lg:flex-row">
            
            { playedTime !== '0' ? (
                <div className="flex flex-col justify-start gap-5 w-75 font-thin text-white/70">
                
                  <span className="flex flex-col lg:flex-row gap-2">
                    <div className="w-12 flex items-center justify-center">
                      <FaClock className="w-10 h-10 mt-1"/>
                    </div>
                    <div>
                      <p>Tempo jogado</p>
                      <p><DurationDisplay
                  ...
                  <span className="flex flex-col lg:flex-row gap-2">
                    <div className="w-12 flex items-center justify-center">
                      <FaGamepad className="w-10 h-10 mt-1"/>
                    </div>
                    <div>
                      <p>Sessões jogadas</p>
                      <p>
                        {focusGame?.gameTitle?.playCount || 0}
                      </p>
                    </div>
            ...
            <div className="w-30">
              <ProgressRing
                progress={focusGame?.progress}
                total={focusGame?.definedTrophies.bronze + focusGame?.definedTrophies.silver + focusGame?.definedTrophies.gold + focusGame?.definedTrophies?.platinum}
                earned={focusGame?.earnedTrophies}
                hideValue={false}
                showTrophy={true}
                textSize={20}
                size={110}
      ...
      >
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="w-40">
            ...
            <p className="font-pixel text-3xl gap-1 flex items-center">
              ...
              {focusGame?.metacritc?.metascore}/100
            </p>
          </div>
          ...
          <div className="w-40 text-right">
            ...
            <p className="font-pixel text-3xl">
              {(focusGame?.trophyGroupCount || 1) - 1 <= 1
                ? "Não possui"
                : (focusGame?.trophyGroupCount || 2) - 1}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="w-40 text-right">
            ...
            <p className="font-pixel text-3xl">{focusGame?.trophySetVersion}</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="w-40 text-right">
            ...
            <p className="flex items-end justify-end">
              ...
              >
                {(focusGame?.metacritc?.rated_br || "RP")
                  .toUpperCase()
                  .substring(0, 2)}
              </span>
            </p>
